...
\subparagraph{Les aggrégats}
\label{subp:Les aggrégats}
Un aggrégat est un objet modifiable lors d'une transaction.
Il est généralement repésenté par un classe unique possédant un identifiant unique dans toute l'application mais on peut aussi trouver des représentations d'aggrégats via plusieurs classes.
...
Ces classes contiennent la logique du \ref(subs:Le domaine){domaine} et permettent d'effectuer des contrôles.
\subparagraph{Les repositories (ou dépôts)}
\label{subp:Les repositories (ou dépôts)}
Les différentes instances des aggrégats sont stockées dans un objet appelé repository (dépôt en français).
Ces objets servent d'intermédiaires entre \ref(subs:Le domaine){le domaine} et la base de donnée.
...
\subparagraph{Le domaine}
\label{subp:Le domaine}
...
Une fois que les différents contrôles sur la commande faits, on applique son effet.
